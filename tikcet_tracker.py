    ...
    try:
        ...
        query = f'''
            ...
            inner join email_addresses ea
                            on
                ea.user_id = j.user_id
            inner join issue_statuses is2 
                            on
                i.status_id = is2.id
            where
                1 = 1
                and j.user_id in ('44','45','59','6','41')
                and i.author_id  not in ('44', '45', '59', '6', '41')
                and DATE(j.created_on) >= CURDATE() - INTERVAL 30 DAY
                and is2.name in ('Closed', 'Resolved')
...
def distinct_db_result(db_result):
    list1=db_result.copy()
    for i in db_result:
        for j in db_result:
            if i[1]==j[1] and i[0]!=j[0] and i[5]<j[5]:
                if i in list1:
                    list1.remove(i)
    ...
    for date in date_list:
        ...
        for ticket in db_list:
            ...
            if date == date_string and "irfan" in ticket[8]:
                ...
                shilton_bhai+=1
        irfan_bhai_list.append(irfan_bhai)
        jannat_apu_list.append(jannat_apu)
        afzal_list.append(afzal)
        javeed_bhai_list.append(javeed_bhai)
        shilton_bhai_list.append(shilton_bhai)
...
def graph_builder(date_list,individual):
    ...
    plt.title("Ticket Resolve Count")
...
def build_pivot_table(data_list1,length_list):
    ...
    pivot_table = pd.pivot_table(output,values=["Count"], index=["Name","Total Resolved","Subject"],columns=[], aggfunc={'Count': 'sum', }, fill_value=0, margins=True,
...
def send_mail(sender, receiver, Cc, mail_pass, subject, pivot_table,img):
    mail_body = f'''
        ...
        Greetings.
        <br><br>
        <b>Please see the Bar Chart of one month ticket resolve summary:</b>
        <br>
        <img src="cid:graph">
        <br><br> 
        <br>
        <b>Please see the summary of resolved tickets of today:</b>
        <br><br>
        {pivot_table}
        <br>
        <br> 
...
def ticket_resolve_tracker_main():
    # DB_info
    host = "10.9.0.112"
    database = "redmine"
    user = "afzal"
    ...
    subject = "Ticket Resolve Tracker From Dashboard ("+today+")"
    ...
    sheet_update(data_of_sheet,"Ticket_resolve_tracker")
